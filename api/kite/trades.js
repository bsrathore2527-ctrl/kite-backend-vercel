...
export default async function handler(req,res){
  try{
    ...
    try{
      ...
      if(trades.length){
      	// 🔥 SAVE RAW TRADES TO KV FOR MTM-WORKER
try {
  await kv.set(TRADEBOOK_KEY, JSON.stringify(trades));
  console.log("📦 [trades] Saved tradebook to KV:", trades.length);
} catch (err) {
  console.log("❌ [trades] Error saving tradebook to KV:", err?.message || err);
}
