...
// - Max Loss (realised + unrealised)
// - Trailing loss floor by realised profit (e.g., every ₹5k)
// - 15m revenge cooldown after a loss
// - 3 consecutive losses => day over
// - Profit locks @10% (block new unless admin re-allows), @20% (hard block new)
// - Expiry day overlays: 14:30 guard, 15:15 exit-only
// - Auto-cancel open orders on block; square-off positions if breached (till 15:25)
// - BTST block window 15:25–15:30 when max loss day
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // 3) Loss delta ⇒ cooldown & consecutive-loss count
    ...
    const maxLossAbs = -(Number(s.max_loss_pct ?? 10) / 100) * cap; // -10% of capital
    const step = Number(s.trail_step_profit ?? 5000);               // trail step (₹)
    ...
    // 9) Enforcement sequence

    // A) Day tripped ⇒ cancel + square (till 15:25) + block new
    if (s.tripped_day) {
    ...
    // C) Revenge cooldown window ⇒ cancel open + force flat
    ...
    if (inExpiryExitOnly) {
      ...
      if (realised < 0) {
        s.block_new_orders = true; // after 14:30 & in loss → block new
      } else {
        // in profit: new exposure cap 3% of capital
    ...
    // E) Global new-trade block ⇒ cancel any remaining opens
    if (s.block_new_orders) await cancelAll(openOrders);

    // F) BTST block window (15:25–15:30) on max-loss day
    if (s.tripped_day && now > cutoff && now <= btstBlockEnd) {
      await squareAll();
    }

    // Persist state for visibility & next tick
    s.realised = realised;
    ...
    // Keep 200 so QStash doesn't spam retries; surface the error in body
